package com.jaime.academix.controller;

import com.jaime.academix.dto.request.ApunteRequest;
import com.jaime.academix.dto.response.ApunteResponse;
import com.jaime.academix.dto.response.MensajeResponse;
import com.jaime.academix.domain.Usuario;
import com.jaime.academix.service.ApunteService;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.security.core.annotation.AuthenticationPrincipal;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.multipart.MultipartFile;

import java.io.IOException;

@RestController
@RequestMapping("/api/notes")
@RequiredArgsConstructor
public class ApunteController {

    private final ApunteService apunteService;

    @PostMapping
    public ResponseEntity<ApunteResponse> crearApunte(
            @Valid @RequestBody ApunteRequest request,
            @AuthenticationPrincipal Usuario autor) {
        return new ResponseEntity<>(apunteService.crearApunte(request, autor), HttpStatus.CREATED);
    }

    @GetMapping("/my")
    public ResponseEntity<Page<ApunteResponse>> listarApuntesPropios(
            @AuthenticationPrincipal Usuario autor,
            Pageable pageable) {
        return ResponseEntity.ok(apunteService.listarApuntesPropios(autor, pageable));
    }

    @GetMapping("/public")
    public ResponseEntity<Page<ApunteResponse>> listarApuntesPublicos(Pageable pageable) {
        return ResponseEntity.ok(apunteService.listarApuntesPublicos(pageable));
    }

    @GetMapping("/public/search")
    public ResponseEntity<Page<ApunteResponse>> buscarApuntesPublicos(
            @RequestParam String q,
            Pageable pageable) {
        return ResponseEntity.ok(apunteService.buscarApuntesPublicos(q, pageable));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApunteResponse> obtenerApunte(
            @PathVariable Long id,
            @AuthenticationPrincipal Usuario usuario) {
        return ResponseEntity.ok(apunteService.obtenerApunte(id, usuario));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApunteResponse> editarApunte(
            @PathVariable Long id,
            @Valid @RequestBody ApunteRequest request,
            @AuthenticationPrincipal Usuario autor) {
        return ResponseEntity.ok(apunteService.editarApunte(id, request, autor));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<MensajeResponse> eliminarApunte(
            @PathVariable Long id,
            @AuthenticationPrincipal Usuario usuario) {
        return ResponseEntity.ok(apunteService.eliminarApunte(id, usuario));
    }

    @PatchMapping("/{id}/publish")
    public ResponseEntity<ApunteResponse> publicarApunte(
            @PathVariable Long id,
            @AuthenticationPrincipal Usuario autor) {
        return ResponseEntity.ok(apunteService.publicarApunte(id, autor));
    }

    @PostMapping("/{id}/download")
    public ResponseEntity<ApunteResponse> descargarApunte(
            @PathVariable Long id,
            @AuthenticationPrincipal Usuario usuario) {
        return ResponseEntity.ok(apunteService.descargarApunte(id, usuario));
    }

    @PostMapping("/{id}/upload-file")
    public ResponseEntity<ApunteResponse> subirArchivo(
            @PathVariable Long id,
            @RequestParam("file") MultipartFile archivo,
            @AuthenticationPrincipal Usuario autor) throws IOException {
        return ResponseEntity.ok(apunteService.subirArchivo(id, archivo, autor));
    }
}
